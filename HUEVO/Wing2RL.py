...
class TradingEnv(gym.Env):
    """
    A continuous-action trading environment:
    - Observation: last LOOKBACK OHLCV plus static features (bid, ask, spread, balance, equity, margin, free_margin, num_positions)
    - Action: Continuous vector of length 3:
        action = [pos_scale, tp_offset, sl_offset]
      * pos_scale ∈ [-1, 1]: Desired position scaling (negative = short, positive = long, magnitude = volume)
      * tp_offset ∈ [-1, 1]: TP offset relative to current price (scaled)
      * sl_offset ∈ [-1, 1]: SL offset relative to current price (scaled)

    Positions are adjusted each step based on the action.
    Reward: Change in equity from last step.
    """
    ...
    def __init__(self, df, lookback=60):
        ...
        # Continuous action space: position scaling, TP offset, SL offset
        ...
        # Parameters for TP/SL scaling (example: we map [-1,1] to a certain pip range)
        self.tp_range = 1000.0  # adjust as needed
    ...
    def _get_equity(self):
        ...
        if info:
            ...
            return self.equity  # fallback if no info
    ...
    def _apply_action(self, pos_scale, tp_offset, sl_offset):
        ...
        if tick is None:
            return
        price = tick.ask  # reference price for buy
        direction = 1 if pos_scale >= 0 else -1

        desired_volume = abs(pos_scale) * LOT_SIZE

        # Adjust TP/SL
        # For a buy: TP above price, SL below price
        # For a sell: TP below price, SL above price
        # We'll define them relative to the current price:
        if direction > 0:  # Long
        ...
        # Close or open positions as needed
        if positions is None or len(positions) == 0:
            # No position open, open a new one if desired_volume > 0
            if desired_volume > 0:
                self._open_position(direction, desired_volume, tp_price, sl_price)
        else:
            # There's an existing position, check direction
            pos = positions[0]
            ...
            if desired_volume == 0:
                ...
                # If direction changed, close and open new
                if current_dir != direction:
                    self._close_position(pos.ticket)
                    self._open_position(direction, desired_volume, tp_price, sl_price)
                else:
                    # Same direction, just modify TP/SL if needed
    ...
    def _modify_position(self, ticket, tp, sl):
        # Modify TP/SL of an open position
        request = {
    ...
    def select_action(self, state, eval_mode=False):
        ...
        with torch.no_grad():
            if eval_mode:
                ...
                std = torch.exp(log_std)
                dist = torch.distributions.Normal(mean, std)
                z = mean  # no sampling for evaluation
                action = torch.tanh(z)
            else:
